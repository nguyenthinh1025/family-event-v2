...
import { DeleteDecorAction } from '../../redux/action/DecorAction';
import { GetEventBookerAction } from '../../redux/action/EventBookerAction';
export default function EventBooker () {
    ...
    const PerPageChange = (value) => {
        setSize(value);
        const newPerPage = Math.ceil(arrEventBooker.length / value);
    ...
    const getData = (current, pageSize) => {
        ...
        // return arrEventBooker.slice((current - 1) * pageSize, current * pageSize)
        if (search === "") {
            return arrEventBooker.slice((current - 1) * pageSize, current * pageSize)
        }
        else {
            return arrEventBooker.filter(item => item.fullname.toLowerCase().includes(search.toLowerCase().trim()))
        }
    };
        ...
        <div classname="app-main__inner  pt-2" style={{ width: '100%', margin: '0 auto', paddingTop: '50px', paddingBottom: '50px' }}>
            <div className='container pl-5 pb-5'>
                <div className=' d-flex justify-content-between'>
                    <div className='container display-6 fw-bold title'>Danh Sách Người Dùng</div>
                    {/* <NavLink to='/adddecor'><button className='btn btn-primary' style={{ width: '150px' }}>Add New</button></NavLink> */}
                    <NavLink className='test' to='/addeventbooker'>
                                                        ...
                                                        <tr key={index}>
                                                            ...
                                                            <td>
                                                                <div className='d-flex justify-content-end'>
                                                                    <NavLink to={`eventbook/${data.eventBookerId}`} style={{ cursor: 'pointer' }}><img src='./../../images/edit.svg' width={30} className="mr-4" /></NavLink>
