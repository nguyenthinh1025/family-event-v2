...
export default function Decor () {
    ...
    const { arrDecor } = useSelector(root => root.DecorReducer);
    console.log(arrDecor);
    ...
    const PerPageChange = (value) => {
        setSize(value);
        const newPerPage = Math.ceil(arrDecor.length / value);
    ...
    const getData = (current, pageSize) => {
        ...
        // return arrDecor.slice((current - 1) * pageSize, current * pageSize)
        if (search === "") {
            return arrDecor.slice((current - 1) * pageSize, current * pageSize)
        }
        else {
            return arrDecor.filter(item => item.decorationName.toLowerCase().includes(search.toLowerCase().trim()))
        }
    };
        ...
        <div classname="app-main__inner  pt-2" style={{ width: '100%', margin: '0 auto', paddingTop: '50px', paddingBottom: '50px' }}>
            <div className='container pl-5 pb-5'>
                <div className=' d-flex justify-content-between'>
                    <div className='container display-6 fw-bold title'>Danh Sách Trang Trí</div>
