...
import { GetDecorationProductByIdAction } from '../../redux/action/DecorationProductAction';
export default function EditDecorProduct (props) {
    const { arrDecorationProductById } = useSelector(root => root.DecorationProductReducer);
    console.log(arrDecorationProductById);
    const dispatch = useDispatch()
    const { id } = props.match.params;
    useEffect(() => {
        const action = GetDecorationProductByIdAction(id);
        dispatch(action)
    }, []);
        ...
        <div classname="app-main__inner  pt-2" style={{ width: '100%', margin: '0 auto', paddingTop: '50px', paddingBottom: '50px' }}>
            <div className='container pl-5 pb-5'>
                <div className=' d-flex justify-content-between'>
                    <div className='' style={{ fontSize: '25px' }}><NavLink to='/decorproduct'>Dụng Cụ Trang Trí</NavLink> / <span>Chỉnh sửa dụng cụ trang trí</span> / <span style={{ color: 'red' }}>{ }</span></div>
