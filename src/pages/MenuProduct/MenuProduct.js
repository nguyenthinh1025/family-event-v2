...
import { GetListMenuProductAction } from '../../redux/action/MenuProductAction';
export default function MenuProduct () {
    const dispatch = useDispatch();
    useEffect(() => {
        const action = GetListMenuProductAction();
        dispatch(action)

    }, []);
    const { arrMenuProduct } = useSelector(root => root.MenuProductReducer);
    console.log(arrMenuProduct);
    ...
    const PerPageChange = (value) => {
        setSize(value);
        const newPerPage = Math.ceil(arrMenuProduct.length / value);
    ...
    const getData = (current, pageSize) => {
        ...
        // return arrMenuProduct.slice((current - 1) * pageSize, current * pageSize)
        if (search === "") {
            return arrMenuProduct.slice((current - 1) * pageSize, current * pageSize)
        }
        else {
            return arrMenuProduct.filter(item => item.decorationName.toLowerCase().includes(search.toLowerCase().trim()))
        }
    };
        ...
        <div classname="app-main__inner  pt-2" style={{ width: '100%', margin: '0 auto', paddingTop: '50px', paddingBottom: '50px' }}>
            <div className='container pl-5 pb-5'>
                <div className=' d-flex justify-content-between'>
                    <div className='container display-6 fw-bold '>Danh Sách Sản Phẩm</div>
                    <NavLink to='/addmenuproduct'><button className='btn btn-primary' style={{ width: '150px' }}>Add New</button></NavLink>
                </div>
            </div>
