...
export default function Product () {
    ...
    const PerPageChange = (value) => {
        setSize(value);
        const newPerPage = Math.ceil(arrProduct.length / value);
    ...
    const getData = (current, pageSize) => {
        ...
        // return arrProduct.slice((current - 1) * pageSize, current * pageSize)
        if (search === "") {
            return arrProduct.slice((current - 1) * pageSize, current * pageSize)
        }
        else {
            return arrProduct.filter(item => item.decorationProductName.toLowerCase().includes(search.toLowerCase().trim()))
        }
    };
                                        ...
                                        <tbody>
                                            {
                                                getData(current, size).length === 0 ? <div style={{ margin: '0 auto', fontSize: '30px', paddingTop: '20px', paddingLeft: '20px' }}>Không tìm thấy kết quả phù hợp</div> : getData(current, size).map((data, index) => {
                                                    return (
                                                        <tr key={index}>
                                                            <td>{++index}</td>
                                                            <td>{data.decorationProductName}</td>
                                                            <td>{data.productDetails}</td>
                                                            <td>{data.productSupplier}</td>
                                                            <td><img src={data.productImage} className="rounded " width={100} height={100} alt="hình dụng cụ" /></td>

                                                            <td>
                                                                <div className='d-flex justify-content-end'>
                                                                    <NavLink to={`/editproduct/${data.productId}`} style={{ cursor: 'pointer' }}><img src='./../../images/edit.svg' width={30} className="mr-4" /></NavLink>
                                                                    <div style={{ cursor: 'pointer' }} onClick={async () => {
                                                                        const action = await DeleteProductAction(data.productId)
                                                                        await dispatch(action)
                                                                        toast.error(`Xóa dụng cụ ${data.decorationProductName} thành công`, {
