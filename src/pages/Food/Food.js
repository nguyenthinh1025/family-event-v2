...
export default function Food () {
    ...
    const PerPageChange = (value) => {
        setSize(value);
        const newPerPage = Math.ceil(arrFood.length / value);
    ...
    const getData = (current, pageSize) => {
        ...
        // return arrFood.slice((current - 1) * pageSize, current * pageSize)
        if (search === "") {
            return arrFood.slice((current - 1) * pageSize, current * pageSize)
        }
        else {
            return arrFood.filter(item => item.foodName.toLowerCase().includes(search.toLowerCase().trim()))
        }
    };
        ...
        <div classname="app-main__inner  pt-2" style={{ width: '100%', margin: '0 auto', paddingTop: '50px', paddingBottom: '50px' }}>
            <div className='container pl-5 pb-5'>
                <div className=' d-flex justify-content-between'>
                    <div className='container display-6 fw-bold title'>Danh Sách Đồ Ăn</div>
                    {/* <NavLink to='/addfood'><button className='btn btn-primary' style={{ width: '150px' }}>Add New</button></NavLink> */}
                    <NavLink className='test' to='/addfood'>
                                                        ...
                                                        <tr key={index}>
                                                            ...
                                                            <td>
                                                                <div className='d-flex justify-content-end'>
                                                                    <NavLink to={`/editfood/${data.foodId}`} style={{ cursor: 'pointer' }}><img src='./../../images/edit.svg' width={30} className="mr-4" /></NavLink>
                                                                    <div style={{ cursor: 'pointer' }} onClick={async () => {
                                                                        const action = await DeleteFoodAction(data.foodId)
