...
export default function FoodType () {
    ...
    const PerPageChange = (value) => {
        setSize(value);
        const newPerPage = Math.ceil(arrFoodType.length / value);
    ...
    const getData = (current, pageSize) => {
        ...
        // return arrFoodType.slice((current - 1) * pageSize, current * pageSize)
        if (search === "") {
            return arrFoodType.slice((current - 1) * pageSize, current * pageSize)
        }
        else {
            return arrFoodType.filter(item => item.foodTypeName.toLowerCase().includes(search.toLowerCase().trim()))
        }
    };
        ...
        <div classname="app-main__inner  pt-2" style={{ width: '100%', margin: '0 auto', paddingTop: '50px', paddingBottom: '50px' }}>
            <div className='container pl-5 pb-5'>
                <div className=' d-flex justify-content-between'>
                    <div className='container display-6 fw-bold title'>Danh Sách Loại Đồ Ăn</div>
                    {/* <NavLink to='/addfoodtype'><button className='btn btn-primary' style={{ width: '150px' }}>Add New</button></NavLink> */}
                    <NavLink className='test' to='/addfoodtype'>
                                        ...
                                        <tbody>
                                            {
                                                getData(current, size).length === 0 ? <div style={{ margin: '0 auto', fontSize: '30px', paddingTop: '20px', paddingLeft: '20px' }}>Không tìm thấy kết quả phù hợp</div> : getData(current, size).map((data, index) => {
                                                    return (
                                                        <tr key={index}>
                                                            <td>{++index}</td>
                                                            <td>{data.foodTypeName}</td>
                                                            <td>{data.foodTypeDetail}</td>

                                                            <td>
                                                                <div className='d-flex justify-content-end'>
                                                                    <NavLink to={`/editfoodtype/${data.foodTypeId}`} style={{ cursor: 'pointer' }}><img src='./../../images/edit.svg' width={30} className="mr-4" /></NavLink>
                                                                    <div style={{ cursor: 'pointer' }} onClick={async () => {
                                                                        const action = await DeleteFoodTypeAction(data.foodTypeId)
                                                                        await dispatch(action)
                                                                        toast.error(`Xóa loại đồ ăn ${data.foodTypeName} thành công`, {
